package burp.Application.SSRFExtension.ExtensionMethod;
...
public class SSRFScan extends AAppExtension {
    ...
    public SSRFScan(GlobalVariableReader globalVariableReader,
        ...
        this.setExtensionName("SSRFScan");
    ...
    private void runExtension() {
        String dnsLogBodyContent = null;
        String payload = "http://dnslog-url";
        ...
        // 实际业务处理
        boolean flag = this.ssrfExploitDetection(payload);
        if(flag)
            return;

        dnsLogBodyContent = this.dnsLog.run().getBodyContent();
        // 防止因为dnslog卡导致没有检测到的问题, 这里进行二次检测, 保证不会漏报
        // 睡眠一段时间, 给dnslog一个缓冲时间
        if(dnsLogBodyContent == null) {
            try {
                Thread.sleep(5000);
        ...
        for (int i = 0; i < this.keyArrayList.size(); i++) {
            // dnslog 内容匹配判断
            if (!dnsLogBodyContent.contains(this.keyArrayList.get(i)) && !dnsLogBodyContent.equals("True")) {
    ...
    private boolean ssrfExploitDetection(String payload) {
        ...
        String dnsLogUrl = key+ "."+host+"."+Integer.toString(port) +".ssrf." + this.dnsLog.run().getTemporaryDomainName();


        // 发送请求
        IHttpRequestResponse newHttpRequestResponse = analyzedRequest.makeSSRFHttpRequest(payload.replace("dnslog-url", dnsLogUrl));
        ...
        // dnslog 内容匹配判断
        if (!dnsLogBodyContent.contains(key) && !dnsLogBodyContent.equals("True")) {
            return false;
        }
    ...
    public IScanIssue export() {
        ...
        String issueName = this.yamlReader.getString("application.ssrfExtension.config.issueName");
